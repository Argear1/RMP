...
fun copyFileWithIO(inputFile: String, outputFile: String) {
    try {
        ...
        println("Задание 1: Копирование завершено. Проверьте файл $outputFile.")
...
class UpperCaseDecorator(processor: TextProcessor) : TextDecorator(processor) {
    override fun process(text: String): String = super.process(text).toUpperCase()
}
...
fun main() {
    ...
    println("Задание 2: Результат обработки текста - $result")
    ...
    asyncReadFile("input.txt") // тут будет асинхронное чтение.
}
